...
public class DragReleaseArrow : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (holding)
        {
            ...
            if (gameObject.GetComponent<Photon.Pun.PhotonView>().IsMine)
            {
                //currentArrow.transform.localScale = new Vector3(Utils.DistanceMouseObj(mousePos, gameObject) * .1f, 1, 1); // Length of arrow changes based on mouse distance from player.
